...
/**
 * Get account information by privatekey
 * @param {String} privateKey of the account
 * @returns {Object} [contains privateKey, publicKey, accountAddress and validatorAddress]
 */
gkeys.getAccountByPrivkey = (privateKey) => {
  let account = {};

  const publicKeyHex = crypto.getTmPubKeyByPrivKey(privateKey);
  const publicKey = crypto.bs58Encode(publicKeyHex, _PUB_KEY_ID);
  const acAddress = crypto.getAcAddrByPrivKey(privateKey)
  const vaAddress = crypto.getVaAddrByPrivKey(privateKey)

  account.privateKey  = privateKey;
  account.publicKey = publicKey;
  account.acAddress = acAddress;
  account.vaAddress = vaAddress;
  
  return account;
}

/**
 * Get account information by publickey
 * @param {String} publicKey of the account
 * @returns {Object} [contains publicKey, accountAddress and validatorAddress]
 */
gkeys.getAccountByPubkey = (publicKey) => {
  let account = {};

  const address = crypto.getTmAddressByPubKey(publicKey);
  const acAddress = crypto.bs58Encode(address, _AC_ADDR_ID);
  const vaAddress = crypto.bs58Encode(address, _VA_ADDR_ID);
