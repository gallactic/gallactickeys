...
cxo.deriveKey = (password, salt, option = {}) => {
  ...
  if (option.kdf === 'pbkdf2'){
    return cxo.pbkdf2Sync(password, salt, option);
  }
  else if (option.kdf === 'scrypt'){
    return cxo.scrypt(password, salt, option);
  }
  else {
    throw new Error('KDF method is not available yet. Please use pbkdf2 or scrypt for now');
  }
};
 ...
 * @returns {Buffer} [Secret key derived from password]
 */
cxo.pbkdf2Sync = (password, salt, option = {}) => {
  var prf;
  ...
  const c = constant.crypto.kdf.pbkdf2.c;
  const dklen = constant.crypto.kdf.pbkdf2.dklen;

  return crypto.pbkdf2Sync(
    password, salt,
    option.kdfparams.c || c,
    option.kdfparams.dklen || dklen,
    prf
  )
}

/**
* @param {String or Buffer} password [User supplied password]
* @param {String or Buffer} salt [Randomly generated bytes]
* @param {integer} N [The number of iterations]
* @param {integer} r [Memory factor]
* @param {integer} p [Parallelization factor]
* @param {integer} dklen [The number of bytes to return]
* @returns {Buffer} [Secret key derived from password]
*/

cxo.scrypt = (password, salt, option = {}) =>{
  option.kdfparams = option.kdfparams || {};
  const n = option.kdfparams.n || constant.crypto.kdf.scrypt.n;
  const p = option.kdfparams.p || constant.crypto.kdf.scrypt.p;
  const r = option.kdfparams.r || constant.crypto.kdf.scrypt.r;
  const dklen = option.kdfparams.dklen || constant.crypto.kdf.scrypt.dklen;
...
/**
 * Encode address using bs58 encoding
 ...
 * @returns {String} An address string
 */
