...
/**
 * Get base 58 encoded public key based on given private key
 * @param {String} privateKey a base 58 encoded string
 * @returns {String} a base 58 encoded public key
 */
cxo.getPubKeyByPrivKey = (privateKey) => {
  return cxo.bs58Encode(cxo.getTmPubKeyByPrivKey(privateKey), _PUB_KEY_ID)
}
...
cxo.bs58Encode = (s, type) => {
  ...
  else if (type === _PRV_KEY_ID && !cxo.isTmPrivateKey(s)) {
...
cxo.isAcAddress = (s) => {
  const prefix = s.substring(0, 2);
  if (s.length !== _AC_ADDR_LENGTH || prefix !== _AC_ADDR_PREF) {
    return false;
  }
...
cxo.isVaAddress = (s) => {
  const prefix = s.substring(0, 2);
  if (s.length !== _VA_ADDR_LENGTH || prefix !== _VA_ADDR_PREF) {
    return false
  }
