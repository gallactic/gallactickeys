 ...
 * @return {Object}
 */
cxo.marshal = (derivedKey, privateKey, salt, iv, option = {}) => {
  ...
  keyObject = {
    address: cxo.getAddressByPrivKey(privateKey).slice(2),
    crypto: {
  ...
  if (option.kdf === 'pbkdf2') {
    ...
    keyObject.crypto.kdfparams = {
      c: option.kdfparams.c || cxo.constants.pbkdf2.c,
      dklen: option.kdfparams.dklen || cxo.constants.pbkdf2.dklen,
      prf: option.kdfparams.prf || cxo.constants.pbkdf2.prf,
...
cxo.encrypt = (text, key, iv, algo) => {
  ...
  algo = algo || cxo.constants.cipher;
